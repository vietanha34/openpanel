...
| K2 | **Bug mới phát hiện khi viết tài liệu: wildcard ở đầu làm crash query** | `properties.*.sku` sinh `arrayMap(x -> trim(x), mapValues(mapExtractKeyLike(properties, '*.sku'))) = 'x'`. Đã chạy trên ClickHouse 26.1: `Code: 130 … while converting 'x' to Array(String)`. Nguyên nhân: regex `/\.\*(?=\.|$)/g` trong `transformPropertyKey` (`chart.service.ts`) cần dấu chấm **đứng trước** `*`, nên `*` ở đầu không thành `%`; sau đó `isWildcard = propertyKey.includes('%')` ra false và clause rơi vào nhánh so sánh vô hướng. `wildcard-property-key.test.ts` không có ca này. UI hiện không sinh wildcard ở đầu (router `chart.properties` chỉ viết `.*.` giữa segment và `[*]`), nhưng API nhận được. **Chưa sửa** — task T14 chỉ là tài liệu |
| K3 | Procedure cũ `overview.eventAnalytics` còn trong router | Không component nào trong `apps/start/src` gọi nó nữa. Prompt T1 dặn xoá khi T4 đã merge |
